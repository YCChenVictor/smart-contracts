...
describe("TaskManager", function () {
  ...
  async function deployTaskFixture() {
    ...
    const taskManager = await TaskManager.deploy({ value: ethers.parseEther("5.0") });  // Deploy with 5 ETH in contract
  ...
  describe("markTaskCompleted", function () {
    it("should mark task as completed and transfer Ether to worker", async function () {
        ...
        await taskManager.connect(owner).createTask(backendId, await worker.getAddress(), mockPaymentAmount);
