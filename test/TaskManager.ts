...
describe("TaskManager", function () {
  ...
  describe("markTaskCompleted", function () {
    it("should mark task as completed and transfer Ether to worker", async function () {
      const { taskManager, owner, worker } =
        await loadFixture(deployTaskFixture);
      ...
      const workerBalanceBefore = await hre.ethers.provider.getBalance(
        worker.getAddress(),
      );
      ...
      const workerBalanceAfter = await hre.ethers.provider.getBalance(
        worker.getAddress(),
      );

      // Check that worker's balance has increased by the paymentAmount
      expect(workerBalanceAfter).to.equal(
        workerBalanceBefore + BigInt(mockPaymentAmount),
      );

      // Log worker's final balance (optional)
      console.log(
        "Worker's balance after payment:",
